...
Example usage
-------------
    python vgg19_fine_tune.py create_embeddings --name 'cleaned'

    python vgg19_fine_tune.py train_top_classifier --name 'cleaned'
                                 --lr 0.0001 --epochs 3 --batch_size 32
                                 --l2_reg 0 --dropout_p 0.5
                                 --penultimate_size 256 --save_model=True

    python vgg19_fine_tune.py make_submission_top_classifier --name 'cleaned'
                                 --dropout_p 0.5 --penultimate_size 256

    python vgg19_fine_tune.py fine_tune --name 'cleaned'
    ...
    python vgg19_fine_tune.py make_submission_vgg19 --name 'cleaned'
...
def create_embeddings(name):
    """Returns vgg19 embeddings (outputs of the last conv layer).
...
def make_submission_top_classifier(name, dropout_p, penultimate_size):
    ...
    submission_file = 'vgg19_top_classifier_{:s}.csv'.format(name)
...
def fine_tune(name, name_ext, lr=1e-4, reduce_lr_factor=0.1,
    ...
    log_dir = join(EXPERIMENTS_DIR, 'vgg19_fine_tuned_{:s}'.format(name))
    callbacks = [
